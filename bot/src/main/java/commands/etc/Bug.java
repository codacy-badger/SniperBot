...
public class Bug implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) throws ParseException, IOException {
        ...
        try {
            ...
            CHANNEL = event.getJDA().getGuildById("538530739017220107").getTextChannelById("548739505553932288");

        } catch (Exception e) {

            if (args.length < 1) {
                event.getTextChannel().sendMessage(new EmbedBuilder().setDescription(
                        "If you want to submit a bug or a suggestion, visit (https://github.com/ROMVoid95/SniperBot/issues/new/choose)**."
                ).build()).queue();
                return;
            }
            event.getTextChannel().sendMessage(MSGS.error().setDescription(
                    ...
                    "If you want to submit a bug or a suggestion, please use the public **(https://github.com/ROMVoid95/SniperBot/issues/new/choose)**."
            ).build()).queue();
            return;
        }
        ...
        if (args.length < 1)
            event.getTextChannel().sendMessage(new EmbedBuilder().setDescription(
                    ...
                    "<bug/gussestion>\n" +
                    "<message>```\n\n" +
                    "Otherwise you can visit **(https://github.com/ROMVoid95/SniperBot/issues/new/choose)** to submit a bug or suggestion."
            ).build()).queue();
        else {
            ...
            MESSAGE = event.getTextChannel().sendMessage(new EmbedBuilder()
                    ...
                    .setFooter("Click on the reaction \uD83D\uDC4D to send the message. to cancel just dont click on the reaction. ;)", null)
                    .build()
            ).complete();
