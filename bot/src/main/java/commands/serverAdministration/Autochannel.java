...
public class Autochannel implements Command, Serializable {
    ...
    private void saveVchan(VoiceChannel vc, Guild g) {
        try {
            Main.getMySql();
			PreparedStatement ps = MySql.getConn().prepareStatement(String.format("INSERT INTO autochans (chan, guild) VALUES ('%s', '%s')", vc.getId(), g.getId()));
            ps.execute();
    ...
    public static void load(JDA jda) {
        ...
        try {
            Main.getMySql();
			PreparedStatement ps = MySql.getConn().prepareStatement("SELECT * FROM autochans");
